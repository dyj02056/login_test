...
> **① ~ ⑨ 전부 해결 완료.** 각 항목에 커밋 번호를 표시해 두었습니다.
> `16ebea0`(①) · `b2924d9`(③) · `7d9343c`(④) · `16fb665`(②⑨) · `493b9c7`(⑥⑦) · `1cd18ea`(⑤⑧)
> ⑩ 은 코드 결함이 아니라 운영 주의사항이라 기록만 유지합니다.
...
### ⑤ 가입·재설정 요청에 속도 제한 없음 — **중간 · 코드 검토** → ✅ **해결됨 (`1cd18ea`)**

> `rate_events` 테이블 기준으로 가입은 IP 당 1시간 5회, 재설정은 IP 당 5회 +
> 대상 아이디당 3회로 제한합니다. 가입은 **성공한 건만** 세어 비밀번호 오타로
> 정상 사용자가 막히지 않게 했고, 재설정은 존재하지 않는 아이디로 훑는 것도
> 남용이므로 모든 요청을 셉니다.
> 재검증: 6번째 가입 차단·다른 IP 정상·실패는 미집계·재설정 IP/아이디 제한 모두 확인.
...
### ⑧ 이메일이 검증되지 않음 — **낮음 · 코드 검토** → ✅ **해결됨 (`1cd18ea`)**

> `email_verifications` 토큰 방식으로 인증을 추가하고 대시보드·회원 관리에
> 상태를 표시합니다. **로그인 조건으로는 걸지 않았습니다** — 이메일이 선택
> 입력이고 메일 발송이 아직 꺼져 있어, 강제하면 기존 회원이 전부 잠기기 때문입니다.
> 메일 발송을 켜고 이메일을 필수로 바꾼 뒤에 재검토할 사항입니다.
...
⑩ 은 코드 결함이 아니라 운영 습관에 대한 것이라 기록만 남깁니다.

### 다음에 검토할 만한 것 (이번 분석 범위 밖)

- **메일 발송 활성화** — `RESEND_API_KEY` 를 넣으면 비밀번호 재설정과 이메일 인증이
  관리자 수동 전달 없이 자동으로 동작합니다. 그 뒤에야 이메일 인증을 로그인 조건으로
  거는 것을 검토할 수 있습니다.
- **테스트 코드** — 지금까지의 검증은 전부 일회성 스크립트였습니다.
  회귀를 자동으로 잡으려면 `tests/` 가 필요합니다.
- **2단계 인증(2FA)** — ④ 에서 남은 한계(봇넷 분산 대입)를 실제로 막으려면 필요합니다.
- **`vercel.json`** — 런타임·리전·타임아웃을 명시적으로 고정하고 싶을 때.
