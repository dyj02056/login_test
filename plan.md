...
> **진행 상황**
> 1~4단계 완료 (`16ebea0` `b2924d9` `7d9343c` `16fb665` `493b9c7`), 배포 확인까지 마침.
> 5·6단계 진행 중.
...
```
1. 한 IP 에서 6번째 가입 시도 → 차단
2. 다른 IP 에서는 정상 가입              ★ 정상 사용자가 막히지 않아야 함
3. 가입 실패(비밀번호 불일치)를 여러 번 → 카운트되지 않아야 함
4. 한 IP 에서 재설정 6회 요청 → 차단
5. 같은 아이디로 4회 재설정 요청 → 3회 후 차단
6. 기존 회원가입·재설정 흐름이 그대로 동작
```
...
```
1. 이메일을 넣고 가입 → 미인증 상태로 생성
2. 대시보드에 미인증 안내가 보임
3. 관리자 화면에서 인증 링크 발급 → 접속 → 인증됨
4. 같은 링크 재사용 → 거부
5. 만료된 링크 → 거부
6. 이메일 없이 가입 → 인증 안내가 뜨지 않음
7. 인증 여부와 무관하게 로그인은 정상 동작   ★ 기존 회원이 잠기지 않아야 함
```
